...
namespace BriefingRoom4DCS.GUI.Utils
{
    public class BuilderUtils
    {
        ...
        internal  async Task<IEnumerable<DatabaseEntryInfo>> SearchAircraft(string searchText) => await Typeahead.SearchDB(briefingRoom, briefingRoom.LanguageKey, DatabaseEntryType.UnitFlyableAircraft, searchText);
        internal string GetAircraftDisplayName(string id) => Typeahead.GetDBDisplayName(briefingRoom, briefingRoom.LanguageKey, DatabaseEntryType.UnitFlyableAircraft, id);

        internal  async Task<IEnumerable<DatabaseEntryInfo>> SearchObjectiveType(string searchText) => await Typeahead.SearchDB(briefingRoom, briefingRoom.LanguageKey, DatabaseEntryType.ObjectiveTask, searchText);
        internal string GetObjectiveTypeDisplayName(string id) => Typeahead.GetDBDisplayName(briefingRoom, briefingRoom.LanguageKey, DatabaseEntryType.ObjectiveTask, id);

        internal string GetObjectiveTargetDisplayName(string id) => Typeahead.GetDBDisplayName(briefingRoom, briefingRoom.LanguageKey, DatabaseEntryType.ObjectiveTarget, id);

        internal  async Task<IEnumerable<DatabaseEntryInfo>> SearchCoalition(string searchText) => await Typeahead.SearchDB(briefingRoom, briefingRoom.LanguageKey, DatabaseEntryType.Coalition, searchText);

        internal string GetCoalitionDisplayName(string id) => Typeahead.GetDBDisplayName(briefingRoom, briefingRoom.LanguageKey, DatabaseEntryType.Coalition, id);
