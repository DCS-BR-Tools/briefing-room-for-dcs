...
namespace BriefingRoom4DCS.UpdateService
{
    ...
    public class GitHubReleaseClient : IDisposable
    {
        ...
        public async Task<ReleaseInfo> GetLatestReleaseAsync(CancellationToken cancellationToken = default)
        {
            ...
            if (release != null)
            {
                release.VersionNumber = ParseVersionFromName(release.Name);
            }
        ...
        /// <summary>
        /// Gets the latest stable (non-prerelease) release.
        /// </summary>
        public async Task<ReleaseInfo> GetLatestStableReleaseAsync(CancellationToken cancellationToken = default)
        {
            var releases = await GetReleasesAsync(cancellationToken);
            return releases.FirstOrDefault(r => !r.IsPrerelease && !r.IsDraft);
        }

        /// <summary>
        /// Gets the latest beta (prerelease) release.
        /// </summary>
        public async Task<ReleaseInfo> GetLatestBetaReleaseAsync(CancellationToken cancellationToken = default)
        {
            var releases = await GetReleasesAsync(cancellationToken);
            return releases.FirstOrDefault(r => r.IsPrerelease && !r.IsDraft);
        }
        ...
        /// <summary>
        /// Parses version number from release name.
        /// Expected format: "BriefingRoom-V0.5.602.11" → "0.5.602.11"
        /// </summary>
        private static string ParseVersionFromName(string name)
        {
            if (string.IsNullOrEmpty(name))
                return null;

            // Match patterns like "V0.5.602.11" or "0.5.602.11"
            var match = Regex.Match(name, @"V?(\d+\.\d+\.\d+(?:\.\d+)?)", RegexOptions.IgnoreCase);
