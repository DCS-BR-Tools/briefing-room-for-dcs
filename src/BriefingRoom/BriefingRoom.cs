...
namespace BriefingRoom4DCS
{
    public sealed class BriefingRoom : IBriefingRoom
    {
        ...
        public DatabaseEntryInfo[] GetDatabaseEntriesInfo(string langKey, DatabaseEntryType entryType, string parameter = "")
        {
            switch (entryType)
            {
                case DatabaseEntryType.Airbase:
                    if (string.IsNullOrEmpty(parameter)) // No parameter, return none
                        ...
                        return (from DBEntryAirbase airbase in Database.GetAllEntries<DBEntryAirbase>() where airbase.Theater == parameter.ToLower() select airbase.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();

                case DatabaseEntryType.Situation:
                    if (string.IsNullOrEmpty(parameter)) // No parameter, return none
                        return Array.Empty<DatabaseEntryInfo>();
                    else // A parameter was provided, return all airbases from specified theater
                        return (from DBEntrySituation situation in Database.GetAllEntries<DBEntrySituation>() where situation.Theater == parameter.ToLower() select situation.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();
                case DatabaseEntryType.ObjectiveTarget:
                    if (string.IsNullOrEmpty(parameter)) // No parameter, return none
                        return (from DBEntryObjectiveTarget objectiveTarget in Database.GetAllEntries<DBEntryObjectiveTarget>() select objectiveTarget.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();
                    else
                        return (from DBEntryObjectiveTarget objectiveTarget in Database.GetAllEntries<DBEntryObjectiveTarget>() where Database.GetEntry<DBEntryObjectiveTask>(parameter).ValidUnitCategories.Contains(objectiveTarget.UnitCategory) select objectiveTarget.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();
                case DatabaseEntryType.ObjectiveTargetBehavior:
                    if (string.IsNullOrEmpty(parameter)) // No parameter, return none
                        return (from DBEntryObjectiveTargetBehavior objectiveTargetBehavior in Database.GetAllEntries<DBEntryObjectiveTargetBehavior>() select objectiveTargetBehavior.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();
                    else
                    {
                ...
                case DatabaseEntryType.Coalition:
                    return (from DBEntryCoalition coalition in Database.GetAllEntries<DBEntryCoalition>() select coalition.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();

                case DatabaseEntryType.DCSMod:
                    return (from DBEntryDCSMod dcsMod in Database.GetAllEntries<DBEntryDCSMod>() select dcsMod.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();

                case DatabaseEntryType.MissionFeature:
                    return (from DBEntryFeatureMission missionFeature in Database.GetAllEntries<DBEntryFeatureMission>() select missionFeature.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();

                case DatabaseEntryType.OptionsMission:
                    return (from DBEntryOptionsMission missionFeature in Database.GetAllEntries<DBEntryOptionsMission>() select missionFeature.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();

                case DatabaseEntryType.ObjectiveFeature:
                    return (from DBEntryFeatureObjective objectiveFeature in Database.GetAllEntries<DBEntryFeatureObjective>() select objectiveFeature.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();

                case DatabaseEntryType.ObjectivePreset:
                    return (from DBEntryObjectivePreset objectivePreset in Database.GetAllEntries<DBEntryObjectivePreset>() select objectivePreset.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();

                case DatabaseEntryType.ObjectiveTask:
                    return (from DBEntryObjectiveTask objectiveTask in Database.GetAllEntries<DBEntryObjectiveTask>() select objectiveTask.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();

                case DatabaseEntryType.Theater:
                    return (from DBEntryTheater theater in Database.GetAllEntries<DBEntryTheater>() select theater.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();
                ...
                case DatabaseEntryType.Unit:
                    ...
                    return Database.GetAllEntries<DBEntryJSONUnit>().Where(x => DBEntryDCSMod.CORE_MODS.Contains(x.Module) || string.IsNullOrEmpty(x.Module) || ModList.Contains(x.Module)).Select(x => x.GetDBEntryInfo()).OrderBy(x => x.Category.Get(langKey)).ThenBy(x => x.Name.Get(langKey)).ToArray();
                ...
                case DatabaseEntryType.UnitFlyableAircraft:
                    return (from DBEntryAircraft unitFlyable in Database.GetAllEntries<DBEntryJSONUnit, DBEntryAircraft>() where unitFlyable.PlayerControllable select unitFlyable.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();

                case DatabaseEntryType.WeatherPreset:
                    return (from DBEntryWeatherPreset weatherPreset in Database.GetAllEntries<DBEntryWeatherPreset>() select weatherPreset.GetDBEntryInfo()).OrderBy(x => x.Name.Get(langKey)).ToArray();
            }
        ...
        public DatabaseEntryInfo? GetSingleDatabaseEntryInfo(string langKey, DatabaseEntryType entryType, string id)
        {
        ...
        {
            return MissionGenerator.GenerateRetryable(this, LanguageKey, new MissionTemplate(Database, templateFilePath));
        }

        public DCSMission GenerateMission(MissionTemplate template)
        {
            return MissionGenerator.GenerateRetryable(this, LanguageKey, template);
        }

        public DCSCampaign GenerateCampaign(string templateFilePath)
        {
            return CampaignGenerator.Generate(this, LanguageKey, new CampaignTemplate(this, templateFilePath));
        }

        public DCSCampaign GenerateCampaign(CampaignTemplate template)
        {
            return CampaignGenerator.Generate(this, LanguageKey, template);
        }
        ...
        public void PrintTranslatableWarning(string langKey, string key, params object[] args)
        {
