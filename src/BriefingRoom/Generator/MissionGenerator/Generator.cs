...
namespace BriefingRoom4DCS.Generator.Mission
{
    internal class MissionGenerator
    {
        ...
        internal static DCSMission Generate(IBriefingRoom briefingRoom, string langKey, MissionTemplateRecord template)
        {
            ...
            if (!template.PlayerFlightGroups.Any(x => !x.Hostile))
                throw new BriefingRoomException(briefingRoom.Database, langKey, "NoFullyHostilePlayers");

            Toolbox.CheckObjectiveProgressionLogic(briefingRoom.Database, template, langKey);

            var mission = new DCSMission(briefingRoom.Database, langKey, template);
                ...
                catch (BriefingRoomRawException err)
                {
                    ...
                    else
                    {
                        ...
                        if (fallbackStageIndex <= 0)
                            throw new BriefingRoomException(briefingRoom.Database, langKey, "FailGeneration", err, err.Message);
        ...
        internal static DCSMission GenerateRetryable(IBriefingRoom briefingRoom, string langKey, MissionTemplate template)
        {
            ...
            var mission = Policy
                ...
                .Retry(3)
                .Execute(() => Generate(briefingRoom, langKey, templateRecord));
