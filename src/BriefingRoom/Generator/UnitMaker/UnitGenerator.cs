...
namespace BriefingRoom4DCS.Generator.UnitMaker
{
    ...
    internal static class UnitGenerator
    {
        ...
        {
            if (unitCount <= 0) throw new BriefingRoomException(briefingRoom.Database, "en", "Asking for a zero units");
            if (families.Count <= 0) throw new BriefingRoomException(briefingRoom.Database, "en", "No Unit Families Provided");
            ...
            {
                var airDefenseUnits = GeneratorTools.GetEmbeddedAirDefenseUnits(briefingRoom, mission.LangKey, mission.TemplateRecord, side, families.First().GetUnitCategory(), country != Country.ALL ? country : null);
        ...
        )
        {
            ...
            {
                List<string> airDefenseUnits = GeneratorTools.GetEmbeddedAirDefenseUnits(briefingRoom, mission.LangKey, mission.TemplateRecord, side, unitFamily.GetUnitCategory());
                if (airDefenseUnits.Count > 0)
                {
                    var dCSGroup = CreateGroup(
                            ref mission,
