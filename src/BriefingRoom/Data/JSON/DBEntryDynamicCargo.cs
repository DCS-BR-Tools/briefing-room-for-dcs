...
{
    internal class DBEntryDynamicCargo: DBEntry
    {
        ...
        internal static Dictionary<string, DBEntry> LoadJSON(string filepath, DatabaseLanguage LangDB)
        {
            ...
            {
                var id = weapon.Key;
                itemMap.Add(id, new DBEntryDynamicCargo()
                {
                    ID = id,
                    UIDisplayName = new LanguageString(weapon.Value)
                });
            }
