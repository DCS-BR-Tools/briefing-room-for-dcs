...
namespace BriefingRoom4DCS.Data
{
    internal class DBEntryCoalition : DBEntry
    {
        ...
        internal Tuple<Country, List<string>> GetRandomUnits(IBriefingRoom briefingRoom, string langKey, List<UnitFamily> families, Decade decade, int count, List<string> unitMods, List<string> unitBanList, bool allowLowPolly, bool blockSuppliers, bool allowStatic, Country? requiredCountry = null, bool lowUnitVariation = false, bool allowDefaults = true)
        {
            // Count is zero, return an empty array.
            if (count < 1) throw new BriefingRoomException(briefingRoom.Database, langKey, "AskingForNoUnits");
            ...
            var validUnits = SelectValidUnits(briefingRoom, langKey, families, decade, unitMods, unitBanList, allowLowPolly, blockSuppliers, allowStatic, allowDefaults: allowDefaults);
        ...
        private Dictionary<Country, List<string>> SelectValidUnits(IBriefingRoom briefingRoom, string langKey, List<UnitFamily> families, Decade decade, List<string> unitMods, List<string> unitBanList, bool allowLowPolly, bool blockSuppliers, bool allowStatic, Country[] allyCountries = null, bool allowDefaults = true)
        {
            ...
            if (validUnits.Count == 0)
            {
                if (allyCountries is null && !blockSuppliers)
                    return SelectValidUnits(briefingRoom, langKey, families, decade, unitMods, unitBanList, allowLowPolly, false, allowStatic, GetAllyCountries(decade), allowDefaults);
                ...
                if (!allowLowPolly)
                {
                    ...
                    return SelectValidUnits(briefingRoom, langKey, families, decade, unitMods, unitBanList, true, blockSuppliers, allowStatic, allyCountries, allowDefaults);
                }
                if (allowDefaults)
                    return GetDefaultUnits(briefingRoom, langKey, families, decade, unitMods, unitBanList);
                return null;
            }
        ...
        private Dictionary<Country, List<string>> GetDefaultUnits(IBriefingRoom briefingRoom, string langKey, List<UnitFamily> families, Decade decade, List<string> unitMods, List<string> unitBanList)
        {
